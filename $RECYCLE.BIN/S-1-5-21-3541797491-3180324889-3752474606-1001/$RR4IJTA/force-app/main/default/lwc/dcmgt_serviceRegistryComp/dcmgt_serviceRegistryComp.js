...
import getServices from '@salesforce/apex/dcmgt_ServiceDetailsController.getServiceByTeamId'
import getServiceById from '@salesforce/apex/dcmgt_ServiceDetailsController.getServiceById'
...
export default class Dcmgt_serviceRegistryComp extends LightningElement {
    objectName = SERVICE_OBJECT
    // @track fieldList = [SERVICE_NAME, SERVICE_DATA_CLASSIFICATION, SERVICE_DESCRIPTION, 
    //     /*SERVICE_SERVICE_OWNER,*/ SERVICE_SYSTEM_ACCOUNT]
    @track fieldList = [
    ...
    getServiceDetails(response) {
        ...
        if(response.data) {
            ...
            if(response.data.length <= 0) {
                ...
                console.log('this.servicetMap', this.servicetMap)
                let service
                if(this.pubServiceId && this.servicetMap.has(this.pubServiceId)){
                    service = this.servicetMap.get(this.pubServiceId)
                } else if(this.selectedService && this.servicetMap.has(this.selectedService)){
                    service = this.servicetMap.get(this.selectedService)
                }
                if(service) {
                    console.log('service', service)
                    let selectedServiceId = this.selectedService
                    this.selectedService = service.Id
                    // if(selectedServiceId != this.pubServiceId) {
                    //     this.updateDataPublicationRequest()
                    // }
                    
                    if(service.dcmgt_Service_Owner__c) {
                        this.ServiceAdmin = service.dcmgt_Service_Owner__r.dcmgt_Team_Owner__r.Email
                    }
                    this.ServiceDescription = service.dcmgt_Service_Description__c
                    this.dataClassification = service.dcmgt_Data_Classification__c
                }
            }
            this.updateDataPublicationRequest()
    ...
    updateDataPublicationRequest() {
        console.log(`tenant:${this.selectedTenantId}, selectedService:${this.selectedService}`)
        updatePublicationRequest({publicationRequestId:this.publiactionId,
            tenantId:this.selectedTenantId,serviceId:this.selectedService, dataSourceId:''})
        .then(data=>{
    ...
    connectedCallback() {
        ...
        console.log('this.pubServiceId', this.pubServiceId)
        console.log('this.publiactionId', this.publiactionId)
    }
